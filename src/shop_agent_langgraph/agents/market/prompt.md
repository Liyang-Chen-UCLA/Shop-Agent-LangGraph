...
4. Return exactly one `MarketSelection` with the selected IDs. The runtime delegates selected products to Research Agent concurrently, asks Eval Agent for read-only relation reports, and lets Market aggregation build canonical collections.
</instructions>
